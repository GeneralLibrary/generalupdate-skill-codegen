...
> 审计日期：2026-06-16
> **当前状态：全部模板已针对 NuGet v10.5.0-beta.6 更新，已通过 dotnet build 编译验证（0 errors）**

---

## 版本升级：v10.4.6 稳定版 → v10.5.0-beta.6

套件从 NuGet v10.4.6 稳定版升级到 v10.5.0-beta.6。主要变更：

| 功能 | v10.4.6 (旧) | v10.5.0-beta.6 (新) |
|------|-------------|---------------------|
...
> ✅ **全部更新完成**。所有模板文件已更新为 v10.5.0-beta.6 API。
...
| 🟠 BuildProperties.csproj 版本 | 5.* → 10.5.0-beta.6 | ✅ |
...
### 1. NuGet 类型冲突（v10.5.0-beta.6 ✅ 已解决）

`GeneralUpdate.Common` 独立命名空间在 v10.4.6 中存在于 `GeneralUpdate.Core` 中，与 `GeneralUpdate.Bowl` 冲突。
**v10.5.0-beta.6 已解决**：Bowl 项目不再引用 Core，各自使用独立的 `GeneralUpdate.Bowl` / `GeneralUpdate.Core` 命名空间。
...
### 2. Bowl LaunchAsync（v10.5.0-beta.6 ✅ 已验证）

`Bowl` 类在 v10.5.0-beta.6 中有公开的 `LaunchAsync` 方法：
